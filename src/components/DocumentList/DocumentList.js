...
export function DocumentList({
    ...
    let state = [];
    const $documentList = createDOMElement({tagName: 'div', attrs: [{ attr: 'class', value: 'document-list' }]});
    
    const makeDocumentTree = ({ id, title, documents, margin }) => {

        return `
            <details id="detail-${id}" style="margin-left:${margin}px" ${state.includes(String(id)) && 'open'}>
                ...
                ${
                    documents.length
                        ? documents.map(({ id, title, documents }) => 
                            makeDocumentTree({ id, title, documents, margin: margin + 10 })).join('')
                        : `<p style="margin-left:${margin + 5}px">하위 페이지가 없습니다.</p>`  
                }
            </details>
        `
    }
